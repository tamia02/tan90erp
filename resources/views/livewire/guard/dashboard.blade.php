            ...
            @forelse ($recent as $entry)
                <div class="py-3 flex items-center justify-between gap-3">
                    ...
                    <div class="flex items-center gap-3 shrink-0">
                        <span class="text-xs capitalize" style="color: var(--text-muted);">{{ str_replace('_', ' ', $entry->status) }}</span>
                        <a href="{{ route('guard.entries.show', $entry) }}" wire:navigate class="text-xs font-medium" style="color: var(--brand);">View Activity</a>
                    </div>
                </div>
            @empty

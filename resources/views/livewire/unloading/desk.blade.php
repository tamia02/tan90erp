...
new #[Layout('layouts.app')] class extends Component
{
    ...
    public function completeUnloading(int $gateId): void
    {
        ...
        AuditLogger::log('Unloading completed, sent to QC Check', $gate->gate_no);
    ...
    {
        return [
            'toStart' => GateEntry::where('status', 'validated')->orderBy('created_at')->get(),
            'inProgress' => GateEntry::with('unloadingRecord')->where('status', 'unloading')->orderBy('created_at')->get(),
            'history' => UnloadingRecord::with('gateEntry')->orderByDesc('created_at')->limit(20)->get(),
        ];
    }
}; ?>
    ...
    <div class="flex flex-col gap-2">
        @forelse ($history as $r)
            <div class="rounded-lg border p-4" style="background: var(--surface-3); border-color: var(--border);">
                ...
                <div class="text-xs mt-1" style="color: var(--text-muted);">Started {{ $r->started_at->format('d M, H:i') }}{{ $r->completed_at ? ' · Completed '.$r->completed_at->format('d M, H:i') : '' }}</div>
            </div>
        @empty
